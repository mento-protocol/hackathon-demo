...
contract BrokerDemoTest is Test {
    ...
    function testConstants() public {
        assertEq(brokerDemo.MENTO_ROUTER(), MENTO_ROUTER, "MentoRouter address should match");
        assertEq(brokerDemo.BI_POOL_MANAGER(), BI_POOL_MANAGER, "BiPoolManager address should match");
    ...
    function testSwapCUSDForCEUR() public {
        ...
        uint256 initialCUSDBalance = ERC20(CUSD).balanceOf(TEST_USER);
        uint256 initialCEURBalance = ERC20(CEUR).balanceOf(TEST_USER);
        
        // Approve the router to spend CUSD
        vm.startPrank(TEST_USER);
        ERC20(CUSD).approve(address(brokerDemo), type(uint256).max);
        ...
        uint256 expectedOut = IMentoRouter(MENTO_ROUTER).getAmountOut(amountIn, path);
        
        // Perform the swap
        brokerDemo.swapCUSDForCEUR(amountIn);
        
        // Check final balances
        uint256 finalCUSDBalance = ERC20(CUSD).balanceOf(TEST_USER);
        uint256 finalCEURBalance = ERC20(CEUR).balanceOf(TEST_USER);
        
        // Verify balances changed as expected
        assertEq(finalCUSDBalance, initialCUSDBalance - amountIn, "CUSD balance should decrease by amountIn");
        assertApproxEqRel(
